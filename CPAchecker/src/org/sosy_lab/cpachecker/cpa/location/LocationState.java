...
public class LocationState implements AbstractStateWithLocation, AbstractQueryableState, Partitionable, Serializable {
  ...
  public static class LocationStateFactory {
    ...
    // this flag controls how the checker conducts interprocedural analysis: clone-based or summary based
  ...
  private static class BackwardsLocationState extends LocationState implements AbstractQueryableState, Targetable {
    ...
    public Iterable<CFAEdge> getOutgoingEdges() {
      if (followFunctionCalls) {
        // excluding function summary edge
        // **notice that the this implementation returns entering edges as it is a backward analysis
        return enteringEdges(getLocationNode());

      } else {
        // including summary edges and excluding function call/return edges -- thus we can analyze functions individually
  ...
  public Iterable<CFAEdge> getOutgoingEdges() {
    if (followFunctionCalls) {
      // ** this is a forward analysis, thus the function returns leaving edges
